...
void bubleSort(Patient a[], unsigned long size) {
  bool swapped;
...
/// @param a указатель на массив
/// @param size размер массива
void insertSort(Patient a[], unsigned long size) {
  long j;
...
void merge(Patient a[], unsigned long low,
                        unsigned long mid,
                        unsigned long high) {
  // Определение переменных.
  Patient *b = new Patient[high+1-low];
  unsigned long h = low, i = 0, j = mid+1, k;
...
void mergeSort(Patient a[], unsigned long low,
                            unsigned long high) {
  if (low < high) {
    long mid = (high + low)/2;
    mergeSort(a, low, mid);
